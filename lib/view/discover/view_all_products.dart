      ...
      child: GridView.builder(
        ...
        itemBuilder: (_, index) {
          if (index == productViewModel.products.length) {
            if (index == productViewModel.products.length) {
              if (productViewModel.currentPage.value <
                  productViewModel.totalPages.value) {
                return Padding(
          ...
          return ProductCard(
            weight: product
                .productWeight, // Assuming `weight` exists in `ProductModel`
            ...
            uid:
                product.id.toString(), // Assuming `id` exists in `ProductModel`
            backgroundImage: product.imageUrls,
            productName: product
                .productTitle, // Assuming `productTitle` exists in `ProductModel`
            productDescription:
                product.productDesc, // Assuming `productDescription` exists
            price: product.cost, // Assuming `price` exists in `ProductModel`
            oldPrice:
                product.msrp, // Assuming `oldPrice` exists in `ProductModel`
            discount: "23" ??
                "No Discount", // Assuming `discount` exists in `ProductModel`
          );
        },
      ),
    );
